...
class Schema {
    ...
    public static function parse( xml : XML ) : Schema {
        ...
        for( e in xml.elements ) {
            ...
            case 'xs:complexType': schema.complexType.push( parseComplexType(e) );
            case 'xs:element': schema.elements.push( parseElement(e) );
            case 'xs:simpleType': schema.simpleType.push( parseSimpleType(e) );
            }
        }
        return schema;
    }
	...
	static function parseAnnotation( xml : XML ) : Annotation {
	 ...
	 static function parseAttribute( xml : XML ) : Attribute {
        ...
        for( e in xml.elements ) switch e.name {
            case 'xs:simpleType': attr.simpleType = parseSimpleType(e);
        }
        return attr;
    }
	...
	static function parseComplexType( xml : XML ) : ComplexType {
        ...
        for( e in xml.elements ) {
            switch e.name {
            case 'xs:annotations': complexType.annotations.push( parseAnnotation(e) );
            case 'xs:simpleContent': complexType.simpleContent = parseSimpleContent(e);
            case 'xs:complexContent': complexType.complexContent = parseComplexContent(e);
            case 'xs:sequence': complexType.sequence = parseSequence(e);
            case 'xs:attribute': complexType.attribute.push( parseAttribute(e) );
            case 'xs:choice': complexType.choice = parseChoice(e);
            }
        }
        return complexType;
    }
	...
	static function parseElement( xml : XML ) : Element {
        ...
        for( e in xml.elements ) {
            switch e.name {
            case 'xs:simpleType': element.simpleType = parseSimpleType(e);
            case 'xs:complexType': element.complexType = parseComplexType(e);
            }
        }
        return element;
    }
	...
	static function parseExtension( xml : XML ) : Extension {
        ...
        for( e in xml.elements ) switch e.name {
            case 'xs:attribute': extension.attribute.push( parseAttribute(e) );
        }
        return extension;
    }
	...
	static function parseRestriction( xml : XML ) : Restriction {
        ...
        for( e in xml.elements ) {
            switch e.name {
            case 'xs:attribute': restriction.attribute.push( parseAttribute(e) );
            case 'xs:enumeration': restriction.enumeration.push( parseEnumeration(e) );
            case 'xs:minLength': restriction.minLength = parseMinLength(e);
            case 'xs:maxLength': restriction.maxLength = parseMaxLength(e);
            }
        }
        return restriction;
    }
    ...
    static function parseSequence( xml : XML ) : Sequence {
        ...
        for( e in xml.elements ) switch e.name {
            case 'xs:element': sequence.elements.push( parseElement(e) );
            case 'xs:any': sequence.any.push( parseAny(e) );
            case 'xs:choice': sequence.choice.push( parseChoice(e) );
        }
        return sequence;
    }
    ...
    static function parseSimpleContent( xml : XML ) : SimpleContent {
        ...
        for( e in xml.elements ) {
            switch e.name {
            case 'xs:extension': obj.extension = parseExtension(e);
            case 'xs:restriction': obj.restriction = parseRestriction(e);
            }
        }
        return obj;
    }
    ...
    static function parseSimpleType( xml : XML ) : SimpleType {
        ...
        for( e in xml.elements ) {
            switch e.name {
            case 'xs:restriction': simpleType.restriction.push( parseRestriction(e) );
            }
        }
        return simpleType;
    }
