...
class BOSH {
	...
	public dynamic function output(data:String) {}
	...
	public function restart(callback:XML->Void) {
		...
		sendRequests(createRequest().set('xmlns', XMLNS)
			.set("xmpp:restart", "true")
			.set("xmlns:xmpp", XBOSH) // .set( "xml:lang", "en" )
			.set("to", host));
	}
	...
	function processResponse() {
		responseTimer.stop();
		output(responseQueue.shift().toString());
		resetResponseProcessor();
	}
...
class BOSHRequest {
	...
	public function send(host:String, port:Int, path:String, data:String, onData:String->Void, onError:String->Void) {
		var t = sys.thread.Thread.create(_send);
