...
class Muc {
    ...
    inline function get_jid() return new Jid(room, host, nick);
    ...
    **/
    public function invite(jid: String, reason: String) : Message {
        ...
        m.properties.push(XML.create("x").set("xmlns",'$XMLNS#user')
            .append(XML.create("invite").set("to", '$room@$host')
                .append(XML.create("reason", reason))));
            ...
            for(e in e.elements) {
                ...
                case "status":
                    if(e.has("code")) {
                        ...
                        if(code != null) r.statusCodes.push(code);
                    }
                case "item":
                    r.items.push({
                        jid: e["jid"],
