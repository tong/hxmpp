...
TEST_XMPP_FLAGS = -main TestXMPP $(HX_CP) $(TESTS_CP)
TEST_JABBER_FLAGS = -main TestJabber $(HX_CP) $(TESTS_CP)
...
NDLL_FLAGS = $(NDLL_NEKO_FLAGS) $(NDLL_LOCAL_FLAGS) $(NDLL_OBJECTS) -o $(NDLL)
...
TEMP_NAME = test
TEMP_MAIN = TestServiceDiscovery
...
all :  $(SWF_SOCKETBRIDGE) \
	   ...
	   $(TEMP_JS) \
	   $(TEMP_PHP) \
	   $(TEST_XMPP_JS) $(TEST_XMPP_SWF) $(TEST_XMPP_NEKO) $(TEST_XMPP_CPP) $(TEST_XMPP_PHP) \
